...
export const googleDrive: Cloud = {
  ...
  async createFile(accessToken: string, content: string, modifiedTime: dayjs.Dayjs): Promise<void> {
    const urlBuilder = new URL('https://www.googleapis.com/upload/drive/v3/files');
...
--${MULTIPART_RELATED_BOUNDARY}--`,
  ...
  async findFile(accessToken: string): Promise<{ id: string; modifiedTime: dayjs.Dayjs } | null> {
    const urlBuilder = new URL('https://www.googleapis.com/drive/v3/files');
    urlBuilder.search = new URLSearchParams({
      access_token: accessToken,
  ...
  async readFile(accessToken: string, id: string): Promise<{ content: string }> {
    const urlBuilder = new URL(`https://www.googleapis.com/drive/v3/files/${id}`);
    urlBuilder.search = new URLSearchParams({
      access_token: accessToken,
      alt: 'media',
    }).toString();
  ...
  async writeFile(
    ...
    const urlBuilder = new URL(`https://www.googleapis.com/upload/drive/v3/files/${id}`);
    urlBuilder.search = new URLSearchParams({
      access_token: accessToken,
      uploadType: 'multipart',
    }).toString();
